import { useTexture } from '@react-three/drei'
import { AssetPaths } from '@/pages/GameOne/config.ts'

useTexture.preload(AssetPaths.start)
useTexture.preload(AssetPaths.lanternImage)
useTexture.preload(AssetPaths.plum)
useTexture.preload(AssetPaths.papercut1)
useTexture.preload(AssetPaths.papercut2)
useTexture.preload(AssetPaths.bg)
useTexture.preload(AssetPaths.xiangyun1)
useTexture.preload(AssetPaths.xiangyun2)
useTexture.preload(AssetPaths.long1)
useTexture.preload(AssetPaths.long2)
useTexture.preload(AssetPaths.flower)
useTexture.preload(AssetPaths.firework)
useTexture.preload(AssetPaths.greeting)
useTexture.preload(AssetPaths.particle)
useTexture.preload(AssetPaths.greetingVideo)
